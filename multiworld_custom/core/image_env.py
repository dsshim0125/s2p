...
from multiworld_custom.core.multitask_env import MultitaskEnv
from multiworld_custom.core.wrapper_env import ProxyEnv
from multiworld_custom.envs.env_util import concatenate_box_spaces
...
class ImageEnv(ProxyEnv, MultitaskEnv):
    ...
    def _get_flat_img(self):
        ...
        if self.transpose:
            image_obs = image_obs.transpose() #[h,w,c] -> [c,w,h]
        assert image_obs.shape[0] == self.channels
        return image_obs.flatten()

    def render(self, mode='wrapped', **kwargs):
        if mode == 'wrapped':
