...
from rlkit.torch.slac.network.initializer import initialize_weight
from rlkit.torch.slac.utils import build_mlp, calculate_kl_divergence
...
class Decoder(torch.jit.ScriptModule):
    ...
    def __init__(self, input_dim=288, output_dim=3, std=1.0, image_size = 64):
        ...
        if image_size==64:
            self.net = nn.Sequential(
                ...
                # (256, 4, 4) -> (128, 8, 8)
                nn.ConvTranspose2d(256, 128, 3, 2, 1, 1), # input channel, output channel, kernel, stride, padding, output padding
                nn.LeakyReLU(0.2, inplace=True),
                # (128, 8, 8) -> (64, 16, 16)
...
class Encoder(torch.jit.ScriptModule):
    ...
    def __init__(self, input_dim=3, output_dim=256, image_size = 64):
        super(Encoder, self).__init__()
        if image_size==64:
            self.net = nn.Sequential(
                # (3, 64, 64) -> (32, 32, 32)
                nn.Conv2d(input_dim, 32, 5, 2, 2), # input channel, output channel, kernel, stride, padding, output padding
                nn.LeakyReLU(0.2, inplace=True),
                # (32, 32, 32) -> (64, 16, 16)
...
class LatentModel(torch.jit.ScriptModule):
    ...
    def __init__(
        ...
        # feat(t) = Encoder(x(t))
        self.encoder = Encoder(state_shape[0], feature_dim, image_size=image_size)
        # p(x(t) | z1(t), z2(t))
