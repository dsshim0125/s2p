...
class ReplayBuffer:
    ...
    def _append(self, state_, action_, reward_, done_): # num_seq(8개)씩 뭉탱이로 들어있음
    ...
    def sample_latent(self, batch_size):
        ...
        for i, idx in enumerate(idxes):
            state_[i, ...] = self.state_[idx] # [num_seq+1(8), c, h, w]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0) # [bs, num_seq+1(8), c, h, w]
    ...
    def random_batch(self, batch_size):
        ...
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        # return state_, self.action_[idxes], self.reward_[idxes, -1], self.done_[idxes, -1]
        return dict(observations = state_,
                    actions = self.action_[idxes],
                    rewards = self.reward_[idxes, -1],
                    terminals = self.done_[idxes, -1],
                    )

                    
    def random_batch_for_tsne(self, batch_size):
        """
        Sample trajectories for updating SAC.
        """
        idxes = np.random.randint(low=0, high=self._real_n, size=batch_size)
        state_ = np.empty((batch_size, self.num_sequences + 1, *self.state_shape), dtype=np.uint8)
        for i, idx in enumerate(idxes):
            state_[i, ...] = self.state_[idx]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        # return state_, self.action_[idxes], self.reward_[idxes, -1], self.done_[idxes, -1]
        real_data_dict =  dict(observations = state_,
                                actions = self.action_[idxes],
                                rewards = self.reward_[idxes, -1],
                                terminals = self.done_[idxes, -1],
                                )
        idxes = np.random.randint(low=self._real_n+1, high=self._n, size=batch_size)
        state_ = np.empty((batch_size, self.num_sequences + 1, *self.state_shape), dtype=np.uint8)
        for i, idx in enumerate(idxes):
            state_[i, ...] = self.state_[idx]
        state_ = torch.tensor(state_, dtype=torch.uint8, device=self.device).float().div_(255.0)
        # return state_, self.action_[idxes], self.reward_[idxes, -1], self.done_[idxes, -1]
        gen_data_dict =  dict(observations = state_,
                                actions = self.action_[idxes],
                                rewards = self.reward_[idxes, -1],
                                terminals = self.done_[idxes, -1],
                                )

        return real_data_dict, gen_data_dict
